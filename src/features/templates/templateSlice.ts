...
export interface TemplateStateWithHistory extends StateWithHistory<TemplateState> {};
...
export const templateSlice = createSlice({
    ...
    reducers: {
        ...
        editTemplate: (state, action: PayloadAction<{ id: ID, newTemplate: Template }>) => {
            ...
            return addNewStateGoingForward(state, { ...state.currentState, templates: newTemplates });
        },
        
        deleteTemplate: (state, action: PayloadAction<{ id: ID }>) => {
            const newTemplates = state.currentState.templates.filter(template => template.id !== action.payload.id);
            return addNewStateGoingForward(state, { ...state.currentState, templates: newTemplates });
        },
        undoTemplates: undo,
        redoTemplates: redo
...
export const { addNewTemplate, editTemplate, deleteTemplate, undoTemplates, redoTemplates } = templateSlice.actions;
